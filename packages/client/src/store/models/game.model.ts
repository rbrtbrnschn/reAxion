import { GameDifficulty } from '@reaxion/common/enums';
import { IGame, IGameDifficulty, IReaction } from '@reaxion/common/interfaces';
import {
  action,
  Action,
  computed,
...
export const gameModelV2: GameModelV2 = {
  ...
  onCopyToHistory: thunkOn(
    (actions) => actions.copyToHistory,
    (actions, _, { getState, getStoreState }) => {
      const state = getState();
      const reactionStoreState = getStoreState().reaction;
      const newHistory = [...state.history];
      newHistory[0].reactions = [
        reactionStoreState.reaction as IReaction,
        ...reactionStoreState.history,
      ];
